...
mod tests {
    ...
    fn include_multiple_criteria() {
        ...
        let c = FilterCriteria::new(&["name", "city"]).unwrap();
        assert_eq!(
            filter_json(json, &c).unwrap(),
    ...
    fn include_numeric_value() {
        ...
        let c = FilterCriteria::new(&["x"]).unwrap();
    ...
    fn include_boolean_and_null() {
        ...
        let c = FilterCriteria::new(&["a", "b", "c"]).unwrap();
        assert_eq!(
            filter_json(json, &c).unwrap(),
    ...
    fn include_single_array_index() {
        ...
        let c = FilterCriteria::new(&["h[1].v"]).unwrap();
    ...
    fn include_slice_first_n() {
        ...
        let c = FilterCriteria::new(&["[:2].n"]).unwrap();
    ...
    fn include_slice_range() {
        let json = r#"[{"n": "a"}, {"n": "b"}, {"n": "c"}, {"n": "d"}]"#;
        let c = FilterCriteria::new(&["[1:3].n"]).unwrap();
    ...
    fn out_of_bounds_index_returns_empty() {
        let json = r#"[1, 2, 3]"#;
        let c = FilterCriteria::new(&["[10]"]).unwrap();
    ...
    fn exclude_nested_key() {
        let json = r#"{"customer": {"name": "Tom", "age": 24}}"#;
        let c = FilterCriteria::new(&["customer.age"]).unwrap();
        assert_eq!(
    ...
    fn exclude_nested_all_match() {
        let json = r#"{"customer": {"name": "Tom", "age": 24}}"#;
        let c = FilterCriteria::new(&["customer.age", "customer.name"]).unwrap();
    ...
    fn exclude_entire_subtree() {
        ...
        let c = FilterCriteria::new(&["private"]).unwrap();
        assert_eq!(
            filter_json_exclude(json, &c).unwrap(),
            r#"{"public":"yes"}"#
        );
    ...
    fn exclude_from_array_elements() {
        ...
        let c = FilterCriteria::new(&["[*].secret"]).unwrap();
        assert_eq!(
            filter_json_exclude(json, &c).unwrap(),
            r#"[{"id":1},{"id":2}]"#
        );
    ...
    fn exclude_preserves_all_when_no_match() {
        let json = r#"{"a": 1, "b": 2}"#;
        let c = FilterCriteria::new(&["z"]).unwrap();
    ...
    fn exclude_single_index_element() {
        let json = r#"[1, 2, 3]"#;
        let c = FilterCriteria::new(&["[1]"]).unwrap();
    ...
    fn exclude_field_from_indexed_element() {
        ...
        let c = FilterCriteria::new(&["[1].price"]).unwrap();
        assert_eq!(
            filter_json_exclude(json, &c).unwrap(),
